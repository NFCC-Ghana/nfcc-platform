...
class UnifiedHydrologicalIntelligence:
    ...
    def get_complete_risk_assessment(self, district: str, rainfall_mm: Optional[float] = None) -> Dict:
        ...
        try:
            ...
            inundation_risk = flood_polygons.get_flood_inundation_risk(district, rainfall_mm)
            ...
            recommendations = self._generate_recommendations(
                district, risk_factors, river_status, dam_risk, soil_status, runoff_forecast
            )
    ...
    def _get_fallback_assessment(self, district: str, rainfall_mm: float) -> Dict:
        ...
        return {
            ...
            'recommendations': [
                {'priority': 'LOW', 'action': 'Monitor conditions', 'target': 'All residents', 'timeframe': 'Ongoing'}
            ],
            'details': {'risk_factors': {'factors': {'rainfall': 50, 'river': 30, 'dam': 20, 'soil': 50, 'history': 20}}}
        }

    def _calculate_risk_factors(self, rainfall: Dict, river: Dict, dam: Dict,
                               soil: Dict, history: Dict) -> Dict:
        """Calculate composite risk factors."""
        weights = {'rainfall': 0.25, 'river': 0.20, 'dam': 0.15, 'soil': 0.20, 'history': 0.20}
        factors = {}
        ...
        factors['dam'] = 80 if dam_risk == 'HIGH' else 50 if dam_risk == 'MEDIUM' else 20
    ...
    def _generate_recommendations(self, district: str, risk: Dict, river: Dict,
                                  dam: Dict, soil: Dict, runoff: Dict) -> List[Dict]:

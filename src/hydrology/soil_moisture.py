...
class SoilMoistureEngine:
    ...
    def get_soil_moisture(self, district: str, rainfall_mm: float = None) -> Dict:
        ...
        if rainfall_mm is not None:
            ...
            current_moisture = min(
                soil["saturation_point"] * 0.9, baseline + rain_effect
            )

        random.seed(hash(f"{district}_{datetime.now().date()}") % 2**32)
        variation = random.uniform(-0.015, 0.015)
        current_moisture = max(
            0.05, min(soil["saturation_point"] * 0.9, current_moisture + variation)
        )

        saturation_index = (current_moisture - soil["wilting_point"]) / (
            soil["saturation_point"] - soil["wilting_point"]
        )

...
class ExplainFusionRequest(BaseModel):
    ...
    nsamples: int = Field(64, ge=8, le=400, description="KernelExplainer samples (latency vs fidelity)")
...
async def explain_forecast_endpoint(body: ExplainFusionRequest) -> Dict[str, Any]:
    ...
    summary_parts = [f"{k} ({v:+.2f})" for k, v in ranked if expl.shap_values.get(k, 0.0) != 0.0 or k in fusion.present_sources]
    summary = "Contribution ranking: " + ", ".join(summary_parts) if summary_parts else "No active sources."

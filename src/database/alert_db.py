...
def save_alert(alert_data: Dict) -> str:
    ...
    alert_id = (
        alert_data.get("alert_id") or f"ALT_{datetime.now().strftime('%Y%m%d%H%M%S')}"
    )
...
def get_alert_history(
    district: Optional[str] = None, limit: int = 100, offset: int = 0
) -> List[Dict]:
...
def get_total_alerts_count(location_filter: Optional[str] = None) -> int:
    ...
    if location_filter:
        cursor.execute(
            "SELECT COUNT(*) FROM alerts WHERE district = ?", (location_filter,)
        )
    else:
        cursor.execute("SELECT COUNT(*) FROM alerts")

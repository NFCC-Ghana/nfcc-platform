...
# Import FIXED visual components with explicit text colors
from hackathon.app.modules.v4.visual_components_fixed import (
...
# Import remaining components from original
...
def render_executive_summary(state):
    ...
    with col2:
        ...
        st.markdown(
            ...
            f"<strong style='color:#1a1a2e;'>{summary}</strong>"
            f"</div>",
...
def render_impact_panel(state, district_data):
    ...
    # Affected communities - Visual list using FIXED component with dark text
...
def render_operations_panel(state):
    ...
    # Shelters - Visual status using FIXED component
    ...
    # Resources - Visual status using FIXED component
    ...
    # Evacuation Routes - Visual display using FIXED component
...
def render_ai_decision_center(state):
    ...
    with col1:
        ...
        ">
            ...
            <h2 style="font-size: 22px; margin: 0 0 12px 0; color: #1a1a2e;">{action}</h2>
        </div>
        """,
            unsafe_allow_html=True,
        )

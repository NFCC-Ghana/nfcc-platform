...
def render_timeline(current_risk: float) -> None:
    ...
    timeline = [
        ...
        {"time": "+6 Hours", "risk": min(100, current_risk * 1.15), "status": "EXTREME"},
        {"time": "+12 Hours", "risk": min(100, current_risk * 1.25), "status": "EXTREME"},
        {"time": "+24 Hours", "risk": min(100, current_risk * 1.1), "status": "HIGH"},
        {"time": "+48 Hours", "risk": max(20, current_risk * 0.8), "status": "MODERATE"}
    ]
    
    df = pd.DataFrame(timeline)
    
    colors = ["#ff0000" if r >= 80 else "#ff6600" if r >= 60 else "#ffaa00" if r >= 40 else "#00cc00" for r in df["risk"]]

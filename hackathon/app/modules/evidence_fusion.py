...
class EvidenceFusionEngine:
    ...
    def get_fused_assessment(self) -> Dict:
        ...
        total_confidence = sum(e["confidence"] for e in active_sources) / len(active_sources) if active_sources else 0
...
def render_evidence_fusion() -> None:
    ...
    with col4:
        st.metric("Evidence Quality", f"{assessment['quality_color']} {assessment['evidence_quality']}")
    ...
    for item in assessment['evidence']:
        status_emoji = "🟢" if item["status"] == "active" else "🟡" if item["status"] == "warning" else "🔴"

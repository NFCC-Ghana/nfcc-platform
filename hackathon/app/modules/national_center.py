...
def render_national_center() -> None:
    ...
    st.caption(
        "📊 Data Source: Simulated for demonstration - connect to NFCC API for live data"
    )
    with col2:
    ...
    with col2:
        ...
        for _, row in top5.iterrows():
            ...
            st.markdown(
                f"{color} **{row['name']}**: {row['risk']:.1f}% ({row['status']})"
            )

    # Alert
    if critical > 0:
        st.error(
            f"🚨 **{critical} DISTRICTS IN CRITICAL STATE** - Immediate action required!"
        )
    elif high_risk > 0:
        st.warning(
            f"⚠️ **{high_risk} DISTRICTS IN HIGH RISK** - Prepare for possible escalation"
        )
    else:

...
def render_operational_resilience() -> None:
    ...
    for source in status["data_sources"]:
        status_emoji = (
            ...
            else "🟡" if source["status"] == "warning" else "🔴"
        )
        status_text = (
            "Active"
            if source["status"] == "active"
            else "Degraded" if source["status"] == "warning" else "Unavailable"
        )
    ...
    if active >= 5:
        st.success(
            "✅ System operating at 100% capacity - All critical data sources available"
        )
    elif active >= 3:
        st.warning("⚠️ System degraded - Some data sources using fallback values")
    else:
        st.error(
            "🔴 System critical - Limited data availability, using fallback models"
        )

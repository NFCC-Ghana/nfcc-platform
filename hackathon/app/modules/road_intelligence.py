...
def get_road_flooding_data(district: str, rainfall: float, soil_saturation: float) -> dict:
    ...
    roads = {
        "Accra Central": {
            "high_risk": [
                {"name": "Alajo Main Road", "depth": 0.5, "confidence": 85, "reason": "Verified flooding report + poor drainage"},
                {"name": "Ring Road Central near Kwame Nkrumah Circle", "depth": 0.4, "confidence": 78, "reason": "Low-lying area + rising water reports"},
                {"name": "Kaneshie Market Road", "depth": 0.35, "confidence": 82, "reason": "Blocked drainage + water level rising"},
                {"name": "Odaw River crossing near Graphic Road", "depth": 0.45, "confidence": 80, "reason": "River proximity + flood warning"}
            ],
            "moderate_risk": [
                {"name": "Independence Avenue", "depth": 0.2, "confidence": 65, "reason": "Moderate rainfall accumulation"},
                {"name": "Liberation Road", "depth": 0.15, "confidence": 60, "reason": "Drainage capacity at 70%"}
            ],
            "safe_routes": [
                {"name": "Independence Avenue (alternative)", "reason": "Higher elevation"},
                {"name": "Liberation Road (alternative)", "reason": "Wider drainage capacity"}
            ]
        }
    }
    
    return roads.get(district, {"high_risk": [], "moderate_risk": [], "safe_routes": []})


def render_road_intelligence(district: str, rainfall: float, soil_saturation: float) -> None:

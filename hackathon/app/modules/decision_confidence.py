...
def get_decision_confidence(district: str, risk_score: float) -> Dict:
    ...
    overall_confidence = round(
        (evidence_confidence + model_confidence + historical_accuracy + data_freshness)
        / 4,
        1,
    )
...
def render_decision_confidence(district: str, risk_score: float) -> None:
    ...
    color = (
        ...
        else "#ff9800" if confidence["overall_confidence"] >= 60 else "#f44336"
    )
    ...
    if risk_score >= 80:
        ...
        st.success(
            f"✅ NORMAL - No immediate risk (Confidence: {confidence['overall_confidence']:.0f}%)"
        )

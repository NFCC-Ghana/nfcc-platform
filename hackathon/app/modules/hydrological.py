...
def render_hydrological_intelligence(district: str) -> None:
    ...
    for i, river in enumerate(rivers[:4]):
        with cols[i]:
            status_color = "🟢" if river["status"] == "NORMAL" else "🟡" if river["status"] == "WARNING" else "🔴"
            st.markdown(f"""
    ...
    for i, dam in enumerate(dams):
        with cols[i]:
            risk_color = "🟢" if dam["risk"] == "LOW" else "🟡" if dam["risk"] == "MEDIUM" else "🔴"
            st.markdown(f"""
    ...
    soil_df = pd.DataFrame({
        "District": list(soil_data.keys()),
        "Saturation (%)": list(soil_data.values())
    })

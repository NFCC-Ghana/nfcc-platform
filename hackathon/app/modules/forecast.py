...
def render_forecast(district: str, current_risk: float) -> None:
    ...
    df = pd.DataFrame({
        "Timeframe": [f"{h}h" for h in hours],
        "Risk": risk_values,
        "Trend": trends
    })
    
    # Color based on risk
    colors = ["#ff0000" if r >= 80 else "#ff6600" if r >= 60 else "#ffaa00" if r >= 40 else "#00cc00" for r in risk_values]
    
    fig = px.line(
        df,
        x="Timeframe",
        y="Risk",
        title="Risk Forecast (Next 72 Hours)",
        markers=True
    )

...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Validate fused forecast vs realized risk column."
    )
    ap.add_argument(
        "csv",
        type=Path,
        help="CSV with chirps_risk,glofas_risk,flood_hub_risk (optional) and target",
    )
    ap.add_argument(
        "--target", default="realized_risk", help="Column name for observed 0–100 risk"
    )
    ap.add_argument(
        "--out", type=Path, default=None, help="Optional JSON path to write metrics"
    )
    args = ap.parse_args()

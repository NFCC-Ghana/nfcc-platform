...
def flood_score(precipitation, roll_3d, z_score):
...
class TestFloodRiskScore:
    def test_dry_day_scores_zero(self):
        assert flood_score(0.0, 0.0, 0.0) == 0.0

    def test_score_non_negative(self):
        assert flood_score(0.0, 0.0, -5.0) >= 0.0

    def test_score_capped_at_100(self):
        assert flood_score(500.0, 1000.0, 10.0) <= 100.0

    def test_heavy_rain_high_score(self):
        score = flood_score(45.0, 95.0, 3.8)
        assert score >= 75.0

    def test_score_increases_with_rain(self):
        low = flood_score(5.0, 10.0, 0.5)
        high = flood_score(30.0, 70.0, 2.5)
        assert high > low

...
class VectorSimilarityEngine:
        ...
        self.vectors.append(
            FailureVector(
                ...
                error_message_hash=hashlib.md5(
                    failure.get("error", "").encode()
                ).hexdigest()[:12],
                payload_structure_hash=hashlib.md5(
                    str(sorted(failure.get("payload", {}).keys())).encode()
                ).hexdigest()[:12],
    ...
    def cluster_failures_semantic(self) -> List[Dict[str, Any]]:
        ...
        return [
            {"cluster_id": k, "size": len(v), "members": v} for k, v in clusters.items()
        ]

    def find_similar_failures(
        self, failure: Dict[str, Any], top_k: int = 5
    ) -> List[Tuple[int, float]]:
        """Find similar failures."""
        target_code = failure.get("status_code", 0)
        similar = [
            (i, 1.0) for i, v in enumerate(self.vectors) if v.error_code == target_code
        ]

...
class DeterminismValidator:
    ...
    def assert_idempotent(client, endpoint: str, payload: Dict[str, Any], n: int = 5):
        ...
        def normalize(data):
            normalized = {k: v for k, v in data.items() if k not in ["timestamp", "time", "request_id"]}
    ...
    @staticmethod
    def replay_failure(client, failure_payload: Dict[str, Any], n: int = 3) -> ReplayResult:
    ...
    @staticmethod
    def golden_path_test(client, golden_payload: Dict[str, Any], expected_response: Dict[str, Any]):
        ...
        for key in expected_response:
            if key in actual:
                if isinstance(expected_response[key], (int, float)):
                    assert abs(actual[key] - expected_response[key]) < 0.01, \
                        f"Field {key}: expected {expected_response[key]}, got {actual[key]}"
                else:
                    assert actual[key] == expected_response[key], \
                        f"Field {key}: expected {expected_response[key]}, got {actual[key]}"

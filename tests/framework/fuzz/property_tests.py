...
class PropertyTestEngine:
    ...
    def valid_payload_strategy(draw):
        ...
        return {
            ...
            "roll_30d": draw(
                st.floats(
                    min_value=0, max_value=200, allow_nan=False, allow_infinity=False
                )
            ),
            "cumulative": draw(
                st.floats(
            ...
            "z_score": draw(
                st.floats(
                    min_value=-5, max_value=10, allow_nan=False, allow_infinity=False
                )
            ),
    ...
    def adversarial_payload_strategy(draw):
        ...
        # Mutate randomly
        mutation_type = draw(
            st.sampled_from(["corrupt", "null", "extreme", "type_mismatch"])
        )
    ...
    def test_monotonicity_invariant(client):
        ...
        def _test(low_precip, high_precip):
            ...
            if low_response.status_code == 200 and high_response.status_code == 200:
                low_score = low_response.json().get(
                    "score", low_response.json().get("risk_score", 0)
                )
                high_score = high_response.json().get(
                    "score", high_response.json().get("risk_score", 0)
                )
                assert (
                    high_score >= low_score
                ), f"Monotonicity violated: {high_score} < {low_score}"

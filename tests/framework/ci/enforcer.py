...
class CIEnforcer:
    ...
    def evaluate(self, results: List[Dict[str, Any]]) -> bool:
        ...
        if len(failures) > self.threshold_failures:
            print(
                f"❌ CI BLOCKED: {len(failures)} resilience failures detected (threshold: {self.threshold_failures})"
            )
            return False
    ...
    def evaluate_coverage(self, coverage_report: Dict[str, float]) -> bool:
        ...
        if total_coverage < self.coverage_threshold:
            print(
                f"❌ CI BLOCKED: Coverage {total_coverage}% below threshold {self.coverage_threshold}%"
            )
            return False

        print(
            f"✅ Coverage {total_coverage}% meets threshold {self.coverage_threshold}%"
        )
        return True

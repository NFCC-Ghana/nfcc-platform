...
class AsyncLoadEngine:
    ...
    async def run_load_test(
        ...
        return AsyncLoadResult(
            ...
            rps=round(rps, 2),
            p50_latency_ms=(
                round(statistics.median(latencies_sorted), 2) if latencies else 0
            ),
            p95_latency_ms=(
                round(latencies_sorted[int(len(latencies_sorted) * 0.95)], 2)
                if latencies
                else 0
            ),
            p99_latency_ms=(
                round(latencies_sorted[int(len(latencies_sorted) * 0.99)], 2)
                if latencies
                else 0
            ),
            error_rate=(
                round(errors / total_requests * 100, 2) if total_requests > 0 else 0
            ),
        )
    ...
    ) -> List[AsyncLoadResult]:
        """Run ramp-up load test."""
        results = []
        for users in range(start_users, end_users + 1, step):
            result = await self.run_load_test(
                endpoint, payload, users, duration_per_step
            )
    ...
    async def run_spike_test(
        ...
        recovery = await self.run_load_test(
            endpoint, payload, base_users, recovery_duration
        )
    ...
    def run_sync_wrapper(
        ...
        return asyncio.run(
            self.run_load_test(endpoint, payload, concurrent_users, duration_seconds)
        )
